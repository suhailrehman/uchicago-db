...
if __name__ == "__main__":
    ...
    for entry in bib_database.entries:
        ...
        if os.path.isfile(filenm):
            ...
            if not os.path.exists(os.path.dirname(filenm)):
                try:
            ...
            with open(filenm, 'w', encoding="utf8") as the_file:
                ...
                db.entries =[entry]
                writer = BibTexWriter()
                bibfilenm='static/files/citations/'+supetrim(entry['ID']+'.bib')

                if not os.path.exists(os.path.dirname(bibfilenm)):
                    try:
                        os.makedirs(os.path.dirname(bibfilenm))
                    except OSError as exc: # Guard against race condition
                        if exc.errno != errno.EEXIST:
                            raise

                with open(bibfilenm, 'w', encoding="utf8") as bibfile:
